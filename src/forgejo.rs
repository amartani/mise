...
pub struct ForgejoAsset {
    ...
    pub url: String,
    /// SHA256 digest (Forgejo/Gitea may provide checksums)
    #[serde(default)]
...
pub static API_URL: &str = "https://codeberg.org/api/v1";
...
async fn get_release_cache(key: &str) -> RwLockReadGuard<'_, CacheGroup<ForgejoRelease>> {
    RELEASE_CACHE
        .write()
        .await
        .entry(key.to_string())
...
pub fn get_headers<U: IntoUrl>(url: U) -> HeaderMap {
    ...
    // Check for Codeberg (default Forgejo instance)
    if url.host_str() == Some("codeberg.org") {
        if let Some(token) = env::CODEBERG_TOKEN.as_ref() {
            set_headers(token);
        }
    } else if let Some(token) = env::MISE_FORGEJO_TOKEN.as_ref() {
        // Generic Forgejo token for other instances
        set_headers(token);
    }
